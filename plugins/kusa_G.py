...
async def _(session: CommandSession):
    ...
    if not (eastGAmount or southGAmount or northGAmount or zhuhaiGAmount or shenzhenGAmount):
        ...
        eValue, sValue, nValue, zValue, szValue = (
            int(eastGAmount * gValues.eastValue), int(southGAmount * gValues.southValue),
            int(northGAmount * gValues.northValue), int(zhuhaiGAmount * gValues.zhuhaiValue),
            int(shenzhenGAmount * gValues.shenzhenValue))
        st += f'您的具体持仓如下：\n'
        st += (f'东校区： {eastGAmount}G * {gValues.eastValue:.3f} = {eValue:,}草\n' if eastGAmount else '')
        st += (f'南校区： {southGAmount}G * {gValues.southValue:.3f} = {sValue:,}草\n' if southGAmount else '')
        st += (f'北校区： {northGAmount}G * {gValues.northValue:.3f} = {nValue:,}草\n' if northGAmount else '')
        st += (f'珠海校区： {zhuhaiGAmount}G * {gValues.zhuhaiValue:.3f} = {zValue:,}草\n' if zhuhaiGAmount else '')
        st += (f'深圳校区： {shenzhenGAmount}G * {gValues.shenzhenValue:.3f} = {szValue:,}草\n' if shenzhenGAmount else '')
...
@scheduled_job('cron', minute='*/30', misfire_grace_time=None)
...
@scheduled_job('cron', hour='23', minute='45', misfire_grace_time=None)
...
@scheduled_job('cron', hour='23', minute='50', misfire_grace_time=None)
